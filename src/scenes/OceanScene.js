...
export class OceanScene {
  ...
  _onPriceUpdate(price) {
    ...
    const ratio = range > 0 ? (price - expandedMin) / range : 0.5
  ...
  _drawSurface() {
    this.surfaceGraphic = new PIXI.Graphics()
  ...
  _updateSurface(time) {
    ...
    g.beginFill(0x0a3a5a, 0.6)
    g.drawPolygon([0, 0, ...points, width, 0])
    g.endFill()

    for (let i = 0; i < points.length - 2; i += 2) {
      const shimmer = 0.3 + Math.sin(time * 2 + i * 0.1) * 0.2
      g.lineStyle(1.5, 0x4dd8ff, shimmer)
